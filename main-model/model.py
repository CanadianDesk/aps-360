...
class EquityModel(nn.Module):
    def __init__(self, width_k_bar=8, kernel_size=3, input_height=256, output_height=16, conv_out_channel_list=[4, 16, 64, 256], pool_type='avg'):
        ...
        for i in range(len(conv_out_channel_list)):
            if i == 0:
                self.conv_layers.append(nn.Conv2d(1, conv_out_channel_list[i], kernel_size, padding=1, stride=1))
            else:
                self.conv_layers.append(nn.Conv2d(conv_out_channel_list[i-1], conv_out_channel_list[i], kernel_size, padding=1, stride=1))
        self.final_conv_layer = nn.Conv2d(conv_out_channel_list[-1], 1, kernel_size, padding=1, stride=1) # Final conv layer to reduce channels to 1
