...
def main():
    ...
    commodities = {
        'WTI_Crude': 'CL=F',        # West Texas Intermediate Crude Oil
        'Brent_Crude': 'BZ=F',      # Brent Crude Oil
        'Natural_Gas': 'NG=F',      # Natural Gas
        'RBOB_Gasoline': 'RB=F',    # RBOB Gasoline
        'Heating_Oil': 'HO=F',      # Heating Oil
        'Canadian_Crude_Index': 'CCI'
    }
    
    # Calculate date range (default: last 5 years to today)
    end_date = datetime.now().strftime('%Y-%m-%d')
    start_date = (datetime.now() - timedelta(days=39*365)).strftime('%Y-%m-%d')
    
    # Define output file
    output_dir = 'commodity_data'
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'oil_gas_prices_{start_date}_to_{end_date}.csv')
...
'''
Recommended stocks (Commodaties):
    'TC_Energy': 'TRP.TO',
    'Suncor_Energy': 'SU.TO',
    'Canadian_Natural_Resources': 'CNQ.TO',
    'ExxonMobil': 'XOM',
...
oil_gas_companies = {
    ...
    'Imperial_Oil': 'IMO.TO',
    'Cenovus_Energy': 'CVE.TO',
    'Pembina_Pipeline': 'PPL.TO',
    'Enbridge': 'ENB.TO',
    'Crescent_Point_Energy': 'VRN.TO',
    ...
    'Phillips_66': 'PSX',
    'Valero_Energy': 'VLO',
    'Pioneer_Natural_Resources': 'PXD',
    'Devon_Energy': 'DVN',
    'Hess_Corporation': 'HES',
    'Baker_Hughes': 'BKR',
    'Halliburton': 'HAL',
    'Schlumberger': 'SLB',
    'Kinder_Morgan': 'KMI',
    'Williams_Companies': 'WMB',
    'Chesapeake_Energy': 'CHK',
    'APA_Corporation': 'APA',
    'ONEOK': 'OKE',
    'Devon_Energy': 'DVN'
}
